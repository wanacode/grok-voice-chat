  ...
  const updateTranscript = (
    ...
    setMessages((prev) => {
      ...
      if (isNew || newMessages.length === 0 || newMessages[0].role !== role) {
        ...
        newMessages[0] = {
          ...newMessages[0],
          text: newMessages[0].text + textDelta,
        };
        return newMessages;
      }
    });
      ...
      ws.onmessage = (event) => {
        ...
        switch (data.type) {
          ...
          case "input_audio_buffer.speech_started":
            updateTranscript("user", "", true);
            setStatus("Listening...");
            break;
          case "conversation.item.input_audio_transcription.completed":
            setMessages((prev) => {
              const updated = [...prev];
              if (updated[0] && updated[0].role === "user")
                updated[0].text = data.transcript;
              return updated;
            });
            break;
          case "response.output_audio_transcript.delta":
            updateTranscript("assistant", data.delta, false);
            break;
    ...
    <div className="flex h-screen w-screen bg-white dark:bg-black text-zinc-900 dark:text-zinc-100 transition-colors duration-300 font-sans overflow-hidden">
              ...
              <div className="grid grid-cols-1 gap-2">
                {VOICES.map((v) => (
                  <button
                    ...
                    className={`text-left px-3 py-2 rounded-xl border text-base transition-all flex flex-row gap-x-2 ${
                      voice === v.id
      ...
      <div className="flex-1 flex flex-col bg-white dark:bg-black">
        <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center bg-white/80 dark:bg-black/80 backdrop-blur-md sticky top-0 z-20">
          <span className="text-base font-bold text-zinc-400 uppercase tracking-[0.2em]">
            Live Transcript
          </span>
          <button
            onClick={() => setMessages([])}
            className="p-2 hover:text-red-500 transition-colors text-zinc-400"
          >
            <Trash2 size={16} />
          </button>
        </div>
